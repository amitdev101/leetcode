...
// @version      4.5
// @description  Fetches chat input from Redis, submits it in ChatGPT, intercepts streaming API responses and stores them in Redis, with extensive logging and a toggle button.
...
(function() {
    ...
    // Send the captured response to Redis via your Django API
    function sendResponseToRedis(responseText) {
        logInfo("Sending captured response to Redis...", responseText);
        GM_xmlhttpRequest({
            method: "POST",
            ...
            onload: function(response) {
                logInfo("ChatGPT response saved to Redis.", response.responseText);
            },
            onerror: function(error) {
                logError("Failed to save response to Redis:", error);
            }
        });
    }
                    ...
                    if (url.includes("/backend-api/conversation")) {
                        ...
                        try {
                            ...
                            if (clone.body) {
                                ...
                                // Dispatch custom event with the captured response
    ...
    // Listen for the custom event "capturedResponse" dispatched by the injected script
    window.addEventListener("capturedResponse", function(e) {
        logInfo("Main script received captured response event:", e.detail.response);
