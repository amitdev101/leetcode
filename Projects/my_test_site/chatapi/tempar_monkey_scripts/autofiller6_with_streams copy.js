// ==UserScript==
// @name         ChatGPT Auto Filler with API Capture & Redis Storage (Streaming, Progress Updates)
// @namespace    http://tampermonkey.net/
// @version      4.6
// @description  Fetches chat input from Redis, submits it in ChatGPT, intercepts streaming API responses and stores both the final and progress responses in Redis, with extensive logging and a toggle button.
...
(function() {
    ...
    // New endpoint for progress updates – adjust key as desired.
    const SAVE_PROGRESS_URL = "http://127.0.0.1:8000/api/set/chat_response/";
                            ...
                            if (clone.body) {
                                ...
                                while (!done) {
                                    ...
                                    if (value) {
                                        ...
                                        // Dispatch progress update event with current cumulative result
                                        const progressEvent = new CustomEvent("capturedResponseProgress", { detail: { progress: result } });
                                        window.dispatchEvent(progressEvent);
                                    }
                                }
                                result += decoder.decode();
                                console.log("Captured streaming response body:", result);
                                // Dispatch final captured response event
                                const event = new CustomEvent("capturedResponse", { detail: { response: result } });
    ...
    // Listen for the custom event "capturedResponse" dispatched by the injected script (final result)
    ...
    // Listen for progress update events and update Redis with progress
    window.addEventListener("capturedResponseProgress", function(e) {
        logInfo("Main script received progress update event:", e.detail.progress);
        updateProgressToRedis(e.detail.progress);
    });
